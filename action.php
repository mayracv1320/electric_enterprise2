			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>S/.$pro_price.00
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Añadir al carrito</button>
								</div>
							</div>
						</div>	
		...
		if($count > 0){
			...
			if (mysqli_query($con,$sql)) {
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Su producto se agregó con éxito..!</b>
					</div>
				";
				exit();
			}
...
if (isset($_POST["Common"])) {
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			//display user cart item with "Lista para pagar" button if user is not login
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Lista para pagar" >
							</form>';
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="electric_enterprise@business.com">
							...
							echo   
								'<input type="hidden" name="return" value="https://electric-enterprise.000webhostapp.com/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="https://electric-enterprise.000webhostapp.com/payment_success.php">
									<input type="hidden" name="cancel_return" value="https://electric-enterprise.000webhostapp.com/cancel.php"/>
