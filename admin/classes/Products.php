...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/electric_enterprise/product_images/".$uniqueImageName)) {
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/electric_enterprise/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Producto modificado con éxito..!'];
					}else{
						return ['status'=> 303, 'message'=> 'No se pudo ejecutar la consulta'];
					}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Producto retirado de existencias'];
			}else{
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'ID de categoría no válido'];
		}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Nueva marca agregada con éxito'];
			}else{
				return ['status'=> 303, 'message'=> 'No se pudo ejecutar la consulta'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Marca eliminada'];
			}else{
				return ['status'=> 202, 'message'=> 'No se pudo ejecutar la consulta'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'ID de marca no válido'];
		}
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		if(!empty($_POST['pid'])){
			...
			echo json_encode(['status'=> 303, 'message'=> 'ID de producto no válido']);
			exit();
		}
	}else{
